...
ws = LRE.LinearGSWs(LRE.Indices(exogenous_nbr,
                                      forward_indices,
                                      current_indices,
                                      backward_indices,
                                      static_indices))
...
back_r = 1:n_back
LRE.solve!(results, jacobian, options, ws)
...
for i = back_r
    ...
    for j = 1:(n_forward(ws.ids) - n_both(ws.ids))
        results.g1[ws.ids.purely_forward[j], i] =
            ws.solver_ws.g2[ws.ids.E_columns.E[n_back + j] - n_back, i]
    end
end

n_stat = n_static(ws.ids)
LRE.add_static!(results, jacobian, ws)
@test ws.b11[:, 2:8] ≈ jacobian[1:n_stat, [6, 7, 8, 9, 10, 11, 13]]
@test results.g1[ws.ids.forward, back_r] ≈ ws.solver_ws.g2
@test results.g1[ws.ids.backward, back_r] ≈ ws.solver_ws.g1
...
lu_t = LU(LRE.factorize!(ws.AGplusB_linsolve_ws, copy(LRE.make_AGplusB!(ws.AGplusB, A, G, B, ws)))...)
...
LRE.solve_for_derivatives_with_respect_to_shocks!(results, jacobian, ws)
g1_2 = -AGplusB\jacobian[:, 20:22]
@test results.g1_2 ≈ g1_2


algo = "GS"
ws = LRE.LinearGSWs(LRE.Indices(exogenous_nbr, forward_indices, current_indices, backward_indices, static_indices))
jacobian = make_jacobian(ws)
jacobian_orig = copy(jacobian)
solve!(results, jacobian, options, ws)
@test d_orig * vcat(I(n_backward(ws.ids)), ws.solver_ws.g2[:, 1:n_backward(ws.ids)])*ws.solver_ws.g1 ≈ e_orig * vcat(I(n_backward(ws.ids)), ws.solver_ws.g2[:, 1:n_backward(ws.ids)])
A2 = jacobian[:, 15:19]
A1 = jacobian[:, 6:14]
A0 = jacobian[:, 1:5]
...
algo = "CR"
ws = LRE.LinearCRWs(LRE.Indices(exogenous_nbr, forward_indices, current_indices, backward_indices, static_indices))
